...
export const FavoritesProvider = ({ children }) => {
  ...
  // Load favorites from cookies and server when user changes
  useEffect(() => {
      ...
      if (isAuthenticated && userId) {
        ...
        try {
          ...
          if (response.data.success) {
            ...
            saveFavoritesToCookie(userId, normalized);
            return;
          }
        } catch (error) {
          console.error('Error loading favorites from server:', error);
          console.log('Server favorites unavailable, loading from cookies');
        }
        
        // If server fails, load from cookies
        const cookieFavorites = loadFavoritesFromCookie(userId);
        if (cookieFavorites.length > 0) {
          dispatch({ type: 'LOAD_FAVORITES', payload: cookieFavorites });
        }
      } else {
        // For guests: load from cookies only
        const cookieFavorites = loadFavoritesFromCookie(null);
        dispatch({ type: 'LOAD_FAVORITES', payload: cookieFavorites });
      }
    };
  ...
  // Save to cookies whenever favorites change
  useEffect(() => {
    const userId = user?._id;
    if (state.items.length > 0) {
      saveFavoritesToCookie(userId, state.items);
    }
  ...
  const clearFavorites = () => {
    ...
    const userId = user?._id;
    clearFavoritesFromCookie(userId);
  };
