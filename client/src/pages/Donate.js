...
const Donate = () => {
  ...
  const donors = [
    { name: "Akinyi", avatar: "A" },
    { name: "Simon", avatar: "S" },
    { name: "Walter", avatar: "W" },
    { name: "Liz", avatar: "L" },
    { name: "Naeku", avatar: "N" },
  ];
  ...
  const faqs = [
    ...
    { q: "Can I donate with M-Pesa?", a: "Yes! Till No: 5914787." },
  ...
  try {
    ...
    console.error(err);
    alert("Error connecting to M-Pesa service.");
  }
};
...
<div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
  ...
  <p style={{ color: "#374151", maxWidth: "600px", margin: "0 auto" }}>
    ...
    and create a safer community. Together, we can make a difference.
        ...
        {/* Progress Bar */}
        <ProgressWrapper>
          <ProgressText>${raised} raised of ${goal} goal</ProgressText>
          <ProgressBar>
        ...
        <DonorCarouselWrapper>
          ...
          >
            {donors.map((d, i) => (
              <DonorCard key={i}>
                <DonorAvatar>{d.avatar}</DonorAvatar>
                {d.name}
              </DonorCard>
            ))}
          </Slider>
        ...
        {/* Payment Tabs */}
        <Tabs>
          <Tab active={activeTab === "paypal"} onClick={() => setActiveTab("paypal")}>PayPal</Tab>
          <Tab active={activeTab === "mpesa"} onClick={() => setActiveTab("mpesa")}>M-Pesa</Tab>
          <Tab active={activeTab === "bank"} onClick={() => setActiveTab("bank")}>Bank</Tab>
        </Tabs>

        {activeTab === "paypal" && (
          <PaymentBox>Pay securely via PayPal.</PaymentBox>
        )}
        {activeTab === "mpesa" && (
  <PaymentBox>
    <h4 style={{ color: "#166534", marginBottom: "0.8rem" }}>
    ...
    <p style={{ fontSize: "0.8rem", marginTop: "0.5rem", color: "#374151" }}>
      You’ll receive a prompt on your phone. Enter your M-Pesa PIN to confirm.
        ...
        {activeTab === "bank" && (
          <PaymentBox>Bank Transfer: Equity Bank, Acc: 1234567890.</PaymentBox>
        )}
        ...
        <ImpactSection>
          <ImpactTitle>Impact of Your Gift</ImpactTitle>
          <ul>
            <li>$25 provides school supplies for a girl</li>
            <li>$50 feeds a family for a month</li>
            <li>$100 installs a handwashing station</li>
          </ul>
        </ImpactSection>
