...
import { FaBriefcase, FaUsers, FaStar, FaArrowRight, FaHammer, FaAward, FaChartLine, FaGlobeAfrica, FaWallet, FaRegLightbulb, FaStore, FaChevronDown, FaChevronUp, FaToolbox, FaUser, FaEnvelope, FaPhone, FaCogs, FaCommentDots } from 'react-icons/fa';
import { useState } from 'react';

const leatherStats = [
  { number: 400, label: 'Graduates', icon: FaUsers, description: 'Women and youth trained in leather making and entrepreneurship', color: '#8B5CF6' },
  { number: 60, label: 'Workshops Held', icon: FaHammer, description: 'Leather making workshops and training sessions', color: '#3b82f6' },
  { number: 18, label: 'Communities Served', icon: FaGlobeAfrica, description: 'Communities across Kenya reached by our leather programs', color: '#f59e0b' },
  { number: 95, label: 'Success Rate', icon: FaStar, description: 'Success rate of our graduates in the leather industry', color: '#ef4444' }
];

const programStats = [
  { program: 'Leather Craft', graduates: 200, successRate: 97, avgIncome: 30000, icon: FaHammer },
  { program: 'Product Design', graduates: 120, successRate: 94, avgIncome: 35000, icon: FaBriefcase },
  { program: 'Business Skills', graduates: 80, successRate: 92, avgIncome: 40000, icon: FaWallet }
];

const yearlyImpact = [
  { year: '2019', graduates: 50, communities: 3 },
  { year: '2020', graduates: 100, communities: 7 },
  { year: '2021', graduates: 200, communities: 12 },
  { year: '2022', graduates: 320, communities: 16 },
  { year: '2023', graduates: 400, communities: 18 }
];

const additionalMetrics = [
  { icon: FaAward, number: 8, label: 'Awards Won', description: 'Awards for excellence in leather craft and entrepreneurship', suffix: '+' },
  { icon: FaChartLine, number: 80, label: 'Employment Rate', description: 'Percentage of graduates employed or running businesses', suffix: '%' }
];

const features = [
  { icon: FaHammer, title: 'Leather Crafting', desc: 'Hands-on training in crafting high-quality leather goods.' },
  { icon: FaBriefcase, title: 'Product Design', desc: 'Learn to design innovative and marketable leather products.' },
  { icon: FaWallet, title: 'Business Skills', desc: 'Entrepreneurship and marketing for sustainable income.' },
  { icon: FaStore, title: 'Market Days', desc: 'Opportunities to sell your products at local and regional markets.' },
  { icon: FaRegLightbulb, title: 'Workshops', desc: 'Regular workshops with industry professionals and guest speakers.' }
];

const HeroSection = styled.section`
  position: relative;
  background: linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url('/images/leather-making/leather.jpg') center/cover;
  min-height: 60vh;
  display: flex;
...
const AdditionalMetricLabel = styled.h3`
  font-size: 1.1rem;
  font-weight: 700;
  color: ${({ theme }) => theme.palette.text.primary};
  margin-bottom: 0.5rem;
    ...
    <StyledForm as={motion.form} onSubmit={handleSubmit} noValidate initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
      ...
      <InputWrap>
        ...
        <Select name="interest" value={fields.interest} onChange={handleChange}>
          ...
          <option value="Market">Market Days</option>
...
`;
const results = [
  { icon: FaUsers, title: 'Career Opportunities', desc: 'Graduates secure jobs in leather production or start their own businesses.' },
  { icon: FaStar, title: 'Award Winners', desc: 'Our students have won awards for craftsmanship and innovation.' },
  { icon: FaWallet, title: 'Entrepreneurship', desc: 'Alumni have launched successful leather brands and shops.' },
  { icon: FaHammer, title: 'Craftsmanship', desc: 'Participants master leather crafting and product design.' }
];
...
`;
const faqs = [
  { q: 'Who can apply for the Leather Program?', a: 'Anyone interested in leather craft, product design, or entrepreneurship is welcome to apply.' },
...
const LeatherCTASection = styled.div`
  text-align: center;
  padding: 3rem 1.5rem;
  background: ${({ theme }) => theme.palette.primary.light};
  border-radius: 14px;
  margin-top: 3rem;
`;
const LeatherCTATitle = styled.h2`
  font-size: 1.8rem;
  font-weight: 900;
  color: ${({ theme }) => theme.palette.primary.dark};
  margin-bottom: 1rem;
`;
const LeatherCTADesc = styled.p`
  font-size: 1.08rem;
  color: ${({ theme }) => theme.palette.text.secondary};
  margin-bottom: 2rem;
`;
const LeatherCTAButtonLarge = styled.a`
  display: inline-block;
  background: ${({ theme }) => theme.palette.primary.main};
  color: #fff;
  font-weight: 700;
...
const LeatherContainer = styled.div`
  max-width: 1200px;
  margin: 0 auto;
  padding: 0 2rem;
  position: relative;
...
const Leather = () => (
  <>
    <HeroSection>
      <HeroOverlay />
      <HeroContent>
        <HeroTitle>Leather Making Impact</HeroTitle>
        <HeroSubtitle>
          Empowering women and youth through leather craft, design, and entrepreneurship across Kenya.
        </HeroSubtitle>
    ...
    <FormSection>
      <FormTitle>Apply to Join the Leather Program</FormTitle>
      <ApplicationForm />
    </FormSection>
    <EndResultsSection>
    ...
    <LeatherCTASection>
      <LeatherCTATitle>Ready to Start Your Leather Journey?</LeatherCTATitle>
      <LeatherCTADesc>Apply now or contact us to learn more about the Leather Program. Your future in leather craft begins here!</LeatherCTADesc>
      <LeatherCTAButtonLarge href="/contact">Contact Us</LeatherCTAButtonLarge>
    </LeatherCTASection>
    <LeatherContainer>
      {/* Main Impact Stats */}
      <Section>
        <SectionHeader>
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }}>
            Real stories, real transformation through our leather making and business programs.
          </motion.p>
        </SectionHeader>
        <ImpactGrid>
          {leatherStats.map((stat, index) => (
      ...
      <Section>
        <SectionHeader>
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }}>
            Our leather programs deliver measurable results for all participants.
          </motion.p>
        </SectionHeader>
        <ProgramStatsGrid>
      ...
      <Section>
        <SectionHeader>
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }}>
            Our leather community continues to grow and inspire more women and youth every year.
          </motion.p>
        </SectionHeader>
        <GrowthGrid>
      ...
      <Section>
        <CTASection>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
            <CTATitle>Join Our Leather Community</CTATitle>
            <CTADescription>
              Ready to transform your future through leather craft? Join us and be part of a creative, supportive community.
            </CTADescription>
            <CTAButton href="/get-involved">Get Involved</CTAButton>
          </motion.div>
        </CTASection>
